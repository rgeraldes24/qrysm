...
func feeRecipientIsPresent(_ *types.EvaluationContext, conns ...*grpc.ClientConn) error {
	...
	rpcclient, err := rpc.DialHTTP(fmt.Sprintf("http://127.0.0.1:%d", e2e.TestParams.Ports.Zond1RPCPort))
	if err != nil {
		return err
	}
	...
	*/
	...
	for _, ctr := range blks.BlockContainers {
		if ctr.GetCapellaBlock() != nil {
			...
			if len(payload.FeeRecipient) == 0 || hexutil.Encode(payload.FeeRecipient) == params.BeaconConfig().ZondBurnAddressHex {
