...
func (node *BeaconNode) Start(ctx context.Context) error {
	...
	args := []string{
		...
		fmt.Sprintf("--%s=%s/zond2-beacon-node-%d", cmdshared.DataDirFlag.Name, e2e.TestParams.TestPath, index),
	...
	if config.UseBuilder {
		args = append(args, fmt.Sprintf("--%s=%s:%d", flags.MevRelayEndpoint.Name, "http://127.0.0.1", e2e.TestParams.Ports.Zond1ProxyPort+index))
	}
...
func generateGenesis(ctx context.Context) (state.BeaconState, error) {
	if e2e.TestParams.Zond1GenesisBlock == nil {
		return nil, errors.New("Cannot construct bellatrix block, e2e.TestParams.Zond1GenesisBlock == nil")
	}
	gb := e2e.TestParams.Zond1GenesisBlock
