...
func ConfigToYaml(cfg *BeaconChainConfig) []byte {
	lines := []string{
		...
		fmt.Sprintf("SECONDS_PER_ETH1_BLOCK: %d", cfg.SecondsPerExecutionBlock),
		fmt.Sprintf("ETH1_FOLLOW_DISTANCE: %d", cfg.ExecutionFollowDistance),
		fmt.Sprintf("EPOCHS_PER_ETH1_VOTING_PERIOD: %d", cfg.EpochsPerExecutionVotingPeriod),
