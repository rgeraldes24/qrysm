...
func assertEqualConfigs(t *testing.T, name string, fields []string, expected, actual *params.BeaconChainConfig) {
	...
	// Validator params.
	assert.Equal(t, expected.ExecutionFollowDistance, actual.ExecutionFollowDistance, "%s: Eth1FollowDistance", name)
	...
	assert.Equal(t, expected.EpochsPerExecutionVotingPeriod, actual.EpochsPerExecutionVotingPeriod, "%s: EpochsPerEth1VotingPeriod", name)
...
func TestModifiedE2E(t *testing.T) {
	...
	c.DepositContractAddress = "Q§4242424242424242424242424242424242424242"
