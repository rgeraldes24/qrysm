...
var placeholderFields = []string{"UPDATE_TIMEOUT",
	...
	"MESSAGE_DOMAIN_INVALID_SNAPPY", "MIN_EPOCHS_FOR_BLOCK_REQUESTS", "MAXIMUM_GOSSIP_CLOCK_DISPARITY",
	"MESSAGE_DOMAIN_VALID_SNAPPY", "GOSSIP_MAX_SIZE", "SUBNETS_PER_NODE", "ATTESTATION_SUBNET_COUNT",
	"MAX_CHUNK_SIZE", "ATTESTATION_PROPAGATION_SLOT_RANGE", "ATTESTATION_SUBNET_PREFIX_BITS", "TTFB_TIMEOUT"}
...
func assertEqualConfigs(t *testing.T, name string, fields []string, expected, actual *params.BeaconChainConfig) {
	...
	assert.Equal(t, expected.EpochsPerZond1VotingPeriod, actual.EpochsPerZond1VotingPeriod, "%s: EpochsPerZond1VotingPeriod", name)
