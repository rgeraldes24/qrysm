...
func (s *Store) SaveExecutionChainData(ctx context.Context, data *v1alpha1.ZOND1ChainData) error {
	ctx, span := trace.StartSpan(ctx, "BeaconDB.SaveExecutionChainData")
	defer span.End()

	if data == nil {
		err := errors.New("cannot save nil zond1data")
...
func (s *Store) ExecutionChainData(ctx context.Context) (*v1alpha1.ZOND1ChainData, error) {
	ctx, span := trace.StartSpan(ctx, "BeaconDB.ExecutionChainData")
	defer span.End()

	var data *v1alpha1.ZOND1ChainData
