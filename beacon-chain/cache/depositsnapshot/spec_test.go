...
type testCase struct {
	...
	ExecutionNodeData        *executionNodeData   `yaml:"eth1_data"`
...
func (tc *testCase) UnmarshalYAML(value *yaml.Node) error {
	...
	tc.ExecutionNodeData = raw.ExecutionNodeData
...
func (ed *executionNodeData) UnmarshalYAML(value *yaml.Node) error {
	raw := struct {
...
func TestFinalization(t *testing.T) {
	...
	require.DeepEqual(t, testCases[127].ExecutionNodeData.DepositRoot, originalRoot)
	err = tree.Finalize(int64(testCases[100].ExecutionNodeData.DepositCount-1), testCases[100].ExecutionNodeData.BlockHash, testCases[100].BlockHeight)
	require.NoError(t, err)
	...
	err = tree.Finalize(int64(testCases[105].ExecutionNodeData.DepositCount-1), testCases[105].ExecutionNodeData.BlockHash, testCases[105].BlockHeight)
	require.NoError(t, err)
...
func TestSnapshotCases(t *testing.T) {
	...
	for _, c := range testCases {
		err = tree.Finalize(int64(c.ExecutionNodeData.DepositCount-1), c.ExecutionNodeData.BlockHash, c.BlockHeight)
		require.NoError(t, err)
