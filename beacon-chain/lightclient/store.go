...
type Store struct {
	...
	FinalizedHeader *zondpbv1.BeaconBlockHeader `json:"finalized_header,omitempty"`
	// CurrentSyncCommittee is the sync committees corresponding to the finalized header
	CurrentSyncCommittee *zondpbv1.SyncCommittee `json:"current_sync_committeeu,omitempty"`
	// NextSyncCommittee is the next sync committees corresponding to the finalized header
	NextSyncCommittee *zondpbv1.SyncCommittee `json:"next_sync_committee,omitempty"`
	// BestValidUpdate is the best available header to switch finalized head to if we see nothing else
	BestValidUpdate *zondpbv1.LightClientUpdate `json:"best_valid_update,omitempty"`
	// OptimisticHeader is the most recent available reasonably-safe header
	OptimisticHeader *zondpbv1.BeaconBlockHeader `json:"optimistic_header,omitempty"`
...
func (s *Store) computeForkVersion(types.Epoch) []byte {
...
func (s *Store) validateWrappedUpdate(update *update, currentSlot types.Slot, genesisValidatorsRoot []byte) error {
	...
	if !update.IsFinalityUpdate() {
		...
		if update.FinalizedHeader.Slot == s.Config.GenesisSlot {
			if update.FinalizedHeader.String() != (&zondpbv1.BeaconBlockHeader{}).String() {
		...
		if !trie.VerifyMerkleProof(
			...
			finalizedRoot[:],
			getSubtreeIndex(zondpbv1.FinalizedRootIndex),
	...
	if !update.IsSyncCommiteeUpdate() {
		...
		if !trie.VerifyMerkleProof(
			update.AttestedHeader.StateRoot,
			root[:],
			getSubtreeIndex(zondpbv1.NextSyncCommitteeIndex),
			update.NextSyncCommitteeBranch) {
			return errors.New("sync committee branch is invalid")
		}
	}

	var syncCommittee *zondpbv1.SyncCommittee
	...
	for i := uint64(0); i < syncAggregate.SyncCommitteeBits.Len(); i++ {
		...
		if bit {
			publicKey, err := dilithiumt.PublicKeyFromBytes(syncCommittee.Pubkeys[i])
			if err != nil {
				return err
			}
	...
	if len(syncAggregate.SyncCommitteeSignatures) != len(participantPubkeys) {
		return errors.New("number of sync committee signatures does not match number of participants")
	}

	for i, sig := range syncAggregate.SyncCommitteeSignatures {
		signature, err := dilithiumt.SignatureFromBytes(sig)
		if err != nil {
			return err
		}
		if !signature.Verify(participantPubkeys[i], signingRoot[:]) {
			return fmt.Errorf("sync committee signature[%d] is invalid", i)
		}
	}
...
func (s *Store) applyUpdate(update *zondpbv1.LightClientUpdate) error {
	storePeriod := computeSyncCommitteePeriodAtSlot(s.Config, s.FinalizedHeader.Slot)
...
func (s *Store) ValidateUpdate(lightClientUpdate *zondpbv1.LightClientUpdate,
...
func (s *Store) maybeValidateAndProcessUpdate(lightClientUpdate *zondpbv1.LightClientUpdate,
...
func (s *Store) ProcessUpdate(lightClientUpdate *zondpbv1.LightClientUpdate,
	currentSlot types.Slot, genesisValidatorsRoot []byte) error {
...
func (s *Store) ProcessValidatedUpdate(lightClientUpdate *zondpbv1.LightClientUpdate,
	currentSlot types.Slot, genesisValidatorsRoot []byte) error {
...
func (s *Store) ProcessFinalityUpdate(update *zondpbv1.LightClientFinalityUpdate, currentSlot types.Slot,
...
func (s *Store) ProcessOptimisticUpdate(update *zondpbv1.LightClientOptimisticUpdate, currentSlot types.Slot,
	genesisValidatorsRoot []byte) error {
