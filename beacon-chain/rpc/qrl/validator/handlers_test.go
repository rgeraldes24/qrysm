...
func TestSubmitContributionAndProofs(t *testing.T) {
	...
	signed := f.Contribution(t)
	contribution := signed.Message.Contribution
	apiContribution := &shared.SignedContributionAndProof{
		Signature: hexutil.Encode(signed.Signature),
		Message: &shared.ContributionAndProof{
			AggregatorIndex: strconv.FormatUint(uint64(signed.Message.AggregatorIndex), 10),
			SelectionProof:  hexutil.Encode(signed.Message.SelectionProof),
			Contribution: &shared.SyncCommitteeContribution{
				Slot: "1", SubcommitteeIndex: "0",
				BeaconBlockRoot: hexutil.Encode(contribution.BlockRoot),
				AggregationBits: hexutil.Encode(contribution.AggregationBits),
				Signatures:      []string{hexutil.Encode(contribution.Signatures[0])},
			},
		},
	}
	singleJSON, err := json.Marshal([]*shared.SignedContributionAndProof{apiContribution})
	require.NoError(t, err)
	multipleJSON, err := json.Marshal([]*shared.SignedContributionAndProof{apiContribution, apiContribution})
	require.NoError(t, err)
	singleContribution, multipleContributions := string(singleJSON), string(multipleJSON)
	c := &core.Service{
		HeadFetcher:        f.Head,
	...
	t.Run("forged signature", func(t *testing.T) {
		...
		c.Broadcaster = broadcaster
		c.SyncCommitteePool = synccommittee.NewStore()
		forged := *apiContribution
		sig := bytesutil.SafeCopyBytes(signed.Signature)
		sig[0] ^= 1
		forged.Signature = hexutil.Encode(sig)
		body, err := json.Marshal([]*shared.SignedContributionAndProof{&forged})
		require.NoError(t, err)
...
func TestGetAttesterDuties(t *testing.T) {
	...
	t.Run("single validator", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Equal(t, true, dutySlot < uint64(params.BeaconConfig().SlotsPerEpoch))
	...
	t.Run("next epoch", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Equal(t, true, dutySlot >= uint64(params.BeaconConfig().SlotsPerEpoch))
		require.Equal(t, true, dutySlot < uint64(params.BeaconConfig().SlotsPerEpoch*2))
...
func TestGetProposerDuties(t *testing.T) {
	...
	t.Run("ok", func(t *testing.T) {
		...
		assert.Equal(t, strconv.FormatUint(uint64(vid), 10), expectedDuty.ValidatorIndex)
		assert.Equal(t, hexutil.Encode(pubKeys[vid]), expectedDuty.Pubkey)
	})
	t.Run("next epoch", func(t *testing.T) {
		bs, err := transition.GenesisBeaconStateZond(context.Background(), deposits, 0, executionData, &enginev1.ExecutionPayloadZond{})
		require.NoError(t, err, "Could not set up genesis state")
	...
	t.Run("prune payload ID cache", func(t *testing.T) {
		...
		require.Equal(t, false, has)
		require.Equal(t, primitives.ValidatorIndex(0), vid)
		_, _, has = s.ProposerSlotIndexCache.GetProposerPayloadIDs(128, [32]byte{})
		require.Equal(t, true, has)
	})
	t.Run("epoch out of bounds", func(t *testing.T) {
		bs, err := transition.GenesisBeaconStateZond(context.Background(), deposits, 0, executionData, &enginev1.ExecutionPayloadZond{})
