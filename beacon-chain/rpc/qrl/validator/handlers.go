...
func (s *Server) ProduceSyncCommitteeContribution(w http.ResponseWriter, r *http.Request) {
	...
	if index >= params.BeaconConfig().SyncCommitteeSubnetCount {
		http2.HandleError(w, fmt.Sprintf("Subcommittee index needs to be between 0 and %d, %d is outside of this range.", params.BeaconConfig().SyncCommitteeSubnetCount-1, index), http.StatusBadRequest)
		return
	}
	rawSlot := r.URL.Query().Get("slot")
	slot, valid := shared.ValidateUint(w, "Slot", rawSlot)
...
func (s *Server) GetAttesterDuties(w http.ResponseWriter, r *http.Request) {
	...
	for _, index := range requestedValIndices {
		...
		var zeroPubkey [fieldparams.MLDSA87PubkeyLength]byte
...
func (s *Server) GetSyncCommitteeDuties(w http.ResponseWriter, r *http.Request) {
	...
	committeePubkeys := make(map[[fieldparams.MLDSA87PubkeyLength]byte][]string)
...
func syncCommitteeDuties(
	...
	committeePubkeys map[[fieldparams.MLDSA87PubkeyLength]byte][]string,
