...
func createLightClientUpdate(
	...
	var nextSyncCommittee *v1.SyncCommittee
	...
	if updateAttestedPeriod == updateSignaturePeriod {
		...
		nextSyncCommittee = &v1.SyncCommittee{
		...
		for i := uint64(0); i < syncCommitteeSize; i++ {
			pubKeys[i] = make([]byte, fieldparams.DilithiumPubkeyLength)
		}
		nextSyncCommittee = &v1.SyncCommittee{
			Pubkeys: pubKeys,
		}
...
func NewLightClientBootstrapFromJSON(bootstrapJSON *LightClientBootstrap) (*v1.LightClientBootstrap, error) {
	bootstrap := &v1.LightClientBootstrap{}
	...
	bootstrap.CurrentSyncCommittee = migration.V1Alpha1SyncCommitteeToV1(currentSyncCommittee)
...
func newLightClientUpdateToJSON(input *v1.LightClientUpdate) *LightClientUpdate {
	if input == nil {
		return nil
	}
	...
	if input.NextSyncCommittee != nil {
		nextSyncCommittee = shared.SyncCommitteeFromConsensus(migration.V1SyncCommitteeToV1Alpha1(input.NextSyncCommittee))
	}
