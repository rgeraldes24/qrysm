...
func TestGetValidators(t *testing.T) {
	...
	t.Run("unknown pubkey is ignored", func(t *testing.T) {
		...
		request := httptest.NewRequest(
			http.MethodGet,
			fmt.Sprintf("http://example.com/zond/v1/beacon/states/{state_id}/validators?id=%s&id=%s", hexPubkey, hexutil.Encode([]byte(strings.Repeat("x", fieldparams.BLSPubkeyLength)))),
			nil,
		)
		request = mux.SetURLVars(request, map[string]string{"state_id": "head"})
		writer := httptest.NewRecorder()
...
func TestGetValidatorBalances(t *testing.T) {
	...
	t.Run("unknown pubkey is ignored", func(t *testing.T) {
		...
		request := httptest.NewRequest(
			http.MethodGet,
			fmt.Sprintf("http://example.com/zond/v1/beacon/states/{state_id}/validator_balances?id=%s&id=%s", hexPubkey, hexutil.Encode([]byte(strings.Repeat("x", fieldparams.BLSPubkeyLength)))),
			nil,
		)
		request = mux.SetURLVars(request, map[string]string{"state_id": "head"})
		writer := httptest.NewRecorder()
