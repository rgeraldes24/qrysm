...
func (s *SignedValidatorRegistration) ToConsensus() (*zond.SignedValidatorRegistrationV1, error) {
	...
	if len(sig) != dilithium.CryptoBytes {
		return nil, fmt.Errorf("Signature length was %d when expecting length %d", len(sig), dilithium.CryptoBytes)
	}
...
func (s *ValidatorRegistration) ToConsensus() (*zond.ValidatorRegistrationV1, error) {
	...
	if len(pubKey) != dilithium.CryptoPublicKeyBytes {
		return nil, fmt.Errorf("FeeRecipient length was %d when expecting length %d", len(pubKey), dilithium.CryptoPublicKeyBytes)
	}
...
func (m *SyncCommitteeMessage) ToConsensus() (*zond.SyncCommitteeMessage, error) {
	...
	sig, err := DecodeHexWithLength(m.Signature, dilithium.CryptoBytes)
	if err != nil {
		return nil, NewDecodeError(err, "Signature")
	}
