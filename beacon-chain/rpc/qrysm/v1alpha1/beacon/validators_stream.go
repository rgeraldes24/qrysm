...
func (is *infostream) handleAddValidatorKeys(reqPubKeys [][]byte) error {
	...
	pubKeysMap := make(map[[dilithium.CryptoPublicKeyBytes]byte]bool, len(is.pubKeys))
...
func (is *infostream) handleRemoveValidatorKeys(reqPubKeys [][]byte) {
	...
	pubKeysMap := make(map[[dilithium.CryptoPublicKeyBytes]byte]bool, len(reqPubKeys))
	for _, pubKey := range reqPubKeys {
		pubKeysMap[bytesutil.ToBytes2592(pubKey)] = true
	}
	max := len(is.pubKeys)
...
func (is *infostream) calculateActivationTimeForPendingValidators(res []*zondpb.ValidatorInfo, headState state.ReadOnlyBeaconState, epoch primitives.Epoch) error {
	...
	pendingValidatorsMap := make(map[[dilithium.CryptoPublicKeyBytes]byte]int)
