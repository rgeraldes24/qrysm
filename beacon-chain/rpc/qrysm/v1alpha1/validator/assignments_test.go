...
func TestGetDuties_MultipleKeys_OK(t *testing.T) {
	...
	assert.Equal(t, true, res.CurrentEpochDuties[0].AttesterSlot < params.BeaconConfig().SlotsPerEpoch)
	assert.Equal(t, true, res.CurrentEpochDuties[1].AttesterSlot < params.BeaconConfig().SlotsPerEpoch)
}
