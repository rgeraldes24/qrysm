...
func (vs *Server) StreamDuties(req *zondpb.DutiesRequest, stream zondpb.BeaconNodeValidator_StreamDutiesServer) error {
	if vs.SyncChecker.Syncing() {
	...
	for {
		...
		case ev := <-stateChannel:
			...
			if ev.Type == statefeed.Reorg {
				data, ok := ev.Data.(*zondpbv1.EventChainReorg)
				if !ok {
