	...
	t.Run("Can get builder payload and blobs in Deneb", func(t *testing.T) {
		...
		bid := &zondpb.BuilderBidDeneb{
			...
			BlindedBlobsBundle: &v1.BlindedBlobsBundle{
				KzgCommitments: [][]byte{bytesutil.PadTo([]byte{1}, dilithiumlib.CryptoPublicKeyBytes), bytesutil.PadTo([]byte{4}, dilithiumlib.CryptoPublicKeyBytes)},
				Proofs:         [][]byte{bytesutil.PadTo([]byte{2}, dilithiumlib.CryptoPublicKeyBytes), bytesutil.PadTo([]byte{5}, dilithiumlib.CryptoPublicKeyBytes)},
		...
		sr, err := signing.ComputeSigningRoot(bid, domain)
		require.NoError(t, err)
		sBid := &zondpb.SignedBuilderBidDeneb{
			Message:   bid,
			Signature: sk.Sign(sr[:]).Marshal(),
		}
		vs.BlockBuilder = &builderTest.MockBuilderService{
