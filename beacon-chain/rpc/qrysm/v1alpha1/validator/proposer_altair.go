...
func (vs *Server) setSyncAggregate(ctx context.Context, blk interfaces.SignedBeaconBlock) {
	...
	if err != nil {
		...
		emptySig := [dilithiumlib.CryptoBytes]byte{0xC0}
...
func (vs *Server) getSyncAggregate(ctx context.Context, slot primitives.Slot, root [32]byte) (*zondpb.SyncAggregate, error) {
	...
	for i := uint64(0); i < params.BeaconConfig().SyncCommitteeSubnetCount; i++ {
		...
		if len(c.Signature)%dilithiumlib.CryptoBytes != 0 {
			return nil, fmt.Errorf(
				"combined Signature length is %d is not in the multiple of %d",
				len(c.Signature), dilithiumlib.CryptoBytes)
		}
		for i := 0; i < len(c.Signature)/dilithiumlib.CryptoBytes; i++ {
			offset := i * dilithiumlib.CryptoBytes
			signature := c.Signature[offset : offset+dilithiumlib.CryptoBytes]
	...
	if syncSig == nil {
		var infSig = [dilithiumlib.CryptoBytes]byte{0xC0} // Infinity signature if itself is nil.
		syncSigBytes = infSig[:]
	} else {
