...
type PoolManager interface {
	PendingDilithiumToExecChanges() ([]*qrysmpb.SignedDilithiumToExecutionChange, error)
	DilithiumToExecChangesForInclusion(beaconState state.ReadOnlyBeaconState) ([]*qrysmpb.SignedDilithiumToExecutionChange, error)
	InsertDilithiumToExecChange(change *qrysmpb.SignedDilithiumToExecutionChange)
	MarkIncluded(change *qrysmpb.SignedDilithiumToExecutionChange)
...
type Pool struct {
	lock    sync.RWMutex
	pending doublylinkedlist.List[*qrysmpb.SignedDilithiumToExecutionChange]
	m       map[primitives.ValidatorIndex]*doublylinkedlist.Node[*qrysmpb.SignedDilithiumToExecutionChange]
}
...
func NewPool() *Pool {
	return &Pool{
		pending: doublylinkedlist.List[*qrysmpb.SignedDilithiumToExecutionChange]{},
		m:       make(map[primitives.ValidatorIndex]*doublylinkedlist.Node[*qrysmpb.SignedDilithiumToExecutionChange]),
	}
}

// Copies the internal map and returns a new one.
func (p *Pool) cycleMap() {
	newMap := make(map[primitives.ValidatorIndex]*doublylinkedlist.Node[*qrysmpb.SignedDilithiumToExecutionChange])
	for k, v := range p.m {
		newMap[k] = v
	}
	p.m = newMap
...
func (p *Pool) PendingDilithiumToExecChanges() ([]*qrysmpb.SignedDilithiumToExecutionChange, error) {
	p.lock.RLock()
	defer p.lock.RUnlock()

	result := make([]*qrysmpb.SignedDilithiumToExecutionChange, p.pending.Len())
...
func (p *Pool) DilithiumToExecChangesForInclusion(st state.ReadOnlyBeaconState) ([]*qrysmpb.SignedDilithiumToExecutionChange, error) {
	p.lock.RLock()
	defer p.lock.RUnlock()
	length := int(math.Min(float64(params.BeaconConfig().MaxDilithiumToExecutionChanges), float64(p.pending.Len())))
	result := make([]*qrysmpb.SignedDilithiumToExecutionChange, 0, length)
	node := p.pending.Last()
...
func (p *Pool) InsertDilithiumToExecChange(change *qrysmpb.SignedDilithiumToExecutionChange) {
	p.lock.Lock()
	defer p.lock.Unlock()
...
func (p *Pool) MarkIncluded(change *qrysmpb.SignedDilithiumToExecutionChange) {
	p.lock.Lock()
	defer p.lock.Unlock()
