...
func TestKV_Aggregated_AggregateUnaggregatedAttestations(t *testing.T) {
	...
	att1 := util.HydrateAttestation(&qrysmpb.Attestation{Data: &qrysmpb.AttestationData{Slot: 1}, AggregationBits: bitfield.Bitlist{0b1001}, Signatures: [][]byte{sig1.Marshal()}})
	att2 := util.HydrateAttestation(&qrysmpb.Attestation{Data: &qrysmpb.AttestationData{Slot: 1}, AggregationBits: bitfield.Bitlist{0b1010}, Signatures: [][]byte{sig1.Marshal()}})
	att3 := util.HydrateAttestation(&qrysmpb.Attestation{Data: &qrysmpb.AttestationData{Slot: 1}, AggregationBits: bitfield.Bitlist{0b1100}, Signatures: [][]byte{sig1.Marshal()}})
	att4 := util.HydrateAttestation(&qrysmpb.Attestation{Data: &qrysmpb.AttestationData{Slot: 1}, AggregationBits: bitfield.Bitlist{0b1001}, Signatures: [][]byte{sig2.Marshal()}})
	att5 := util.HydrateAttestation(&qrysmpb.Attestation{Data: &qrysmpb.AttestationData{Slot: 2}, AggregationBits: bitfield.Bitlist{0b1001}, Signatures: [][]byte{sig1.Marshal()}})
	att6 := util.HydrateAttestation(&qrysmpb.Attestation{Data: &qrysmpb.AttestationData{Slot: 2}, AggregationBits: bitfield.Bitlist{0b1010}, Signatures: [][]byte{sig1.Marshal()}})
	att7 := util.HydrateAttestation(&qrysmpb.Attestation{Data: &qrysmpb.AttestationData{Slot: 2}, AggregationBits: bitfield.Bitlist{0b1100}, Signatures: [][]byte{sig1.Marshal()}})
	att8 := util.HydrateAttestation(&qrysmpb.Attestation{Data: &qrysmpb.AttestationData{Slot: 2}, AggregationBits: bitfield.Bitlist{0b1001}, Signatures: [][]byte{sig2.Marshal()}})
	atts := []*qrysmpb.Attestation{att1, att2, att3, att4, att5, att6, att7, att8}
...
func TestKV_Aggregated_SaveAggregatedAttestation(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			att: util.HydrateAttestation(&qrysmpb.Attestation{
				Data: &qrysmpb.AttestationData{}, AggregationBits: bitfield.Bitlist{0b10100}}),
	...
	r, err := hashFn(util.HydrateAttestationData(&qrysmpb.AttestationData{
		Slot: 100,
	}))
	require.NoError(t, err)
...
func TestKV_Aggregated_SaveAggregatedAttestations(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name: "no duplicates",
			atts: []*qrysmpb.Attestation{
				util.HydrateAttestation(&qrysmpb.Attestation{Data: &qrysmpb.AttestationData{Slot: 1},
					AggregationBits: bitfield.Bitlist{0b1101}}),
				util.HydrateAttestation(&qrysmpb.Attestation{Data: &qrysmpb.AttestationData{Slot: 1},
					AggregationBits: bitfield.Bitlist{0b1101}}),
			},
...
func TestKV_Aggregated_DeleteAggregatedAttestation(t *testing.T) {
	t.Run("nil attestation", func(t *testing.T) {
		...
		att := util.HydrateAttestation(&qrysmpb.Attestation{AggregationBits: bitfield.Bitlist{0b10101}, Data: &qrysmpb.AttestationData{Slot: 2}})
		assert.NoError(t, cache.DeleteAggregatedAttestation(att))
	})

	t.Run("non aggregated attestation", func(t *testing.T) {
		cache := NewAttCaches()
		att := util.HydrateAttestation(&qrysmpb.Attestation{AggregationBits: bitfield.Bitlist{0b1001}, Data: &qrysmpb.AttestationData{Slot: 2}})
	...
	t.Run("nonexistent attestation", func(t *testing.T) {
		cache := NewAttCaches()
		att := util.HydrateAttestation(&qrysmpb.Attestation{AggregationBits: bitfield.Bitlist{0b1111}, Data: &qrysmpb.AttestationData{Slot: 2}})
		assert.NoError(t, cache.DeleteAggregatedAttestation(att))
	})

	t.Run("non-filtered deletion", func(t *testing.T) {
		cache := NewAttCaches()
		att1 := util.HydrateAttestation(&qrysmpb.Attestation{Data: &qrysmpb.AttestationData{Slot: 1}, AggregationBits: bitfield.Bitlist{0b11010}})
		att2 := util.HydrateAttestation(&qrysmpb.Attestation{Data: &qrysmpb.AttestationData{Slot: 2}, AggregationBits: bitfield.Bitlist{0b11010}})
		att3 := util.HydrateAttestation(&qrysmpb.Attestation{Data: &qrysmpb.AttestationData{Slot: 3}, AggregationBits: bitfield.Bitlist{0b11010}})
		att4 := util.HydrateAttestation(&qrysmpb.Attestation{Data: &qrysmpb.AttestationData{Slot: 3}, AggregationBits: bitfield.Bitlist{0b10101}})
	...
	t.Run("filtered deletion", func(t *testing.T) {
		cache := NewAttCaches()
		att1 := util.HydrateAttestation(&qrysmpb.Attestation{Data: &qrysmpb.AttestationData{Slot: 1}, AggregationBits: bitfield.Bitlist{0b110101}})
		att2 := util.HydrateAttestation(&qrysmpb.Attestation{Data: &qrysmpb.AttestationData{Slot: 2}, AggregationBits: bitfield.Bitlist{0b110111}})
		att3 := util.HydrateAttestation(&qrysmpb.Attestation{Data: &qrysmpb.AttestationData{Slot: 2}, AggregationBits: bitfield.Bitlist{0b110100}})
		att4 := util.HydrateAttestation(&qrysmpb.Attestation{Data: &qrysmpb.AttestationData{Slot: 2}, AggregationBits: bitfield.Bitlist{0b110101}})
		atts := []*qrysmpb.Attestation{att1, att2, att3, att4}
		require.NoError(t, cache.SaveAggregatedAttestations(atts))
		...
		returned := cache.AggregatedAttestations()
		wanted := []*qrysmpb.Attestation{att1, att2}
		sort.Slice(returned, func(i, j int) bool {
...
func TestKV_Aggregated_HasAggregatedAttestation(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input: util.HydrateAttestation(&qrysmpb.Attestation{
				Data: &qrysmpb.AttestationData{
					Slot: 1,
				},
				AggregationBits: bitfield.Bitlist{0b1111}}),
...
func TestKV_Aggregated_DuplicateAggregatedAttestations(t *testing.T) {
	...
	att1 := util.HydrateAttestation(&qrysmpb.Attestation{Data: &qrysmpb.AttestationData{Slot: 1}, AggregationBits: bitfield.Bitlist{0b1101}, Signatures: [][]byte{sig, sig}})
	att2 := util.HydrateAttestation(&qrysmpb.Attestation{Data: &qrysmpb.AttestationData{Slot: 1}, AggregationBits: bitfield.Bitlist{0b1111}, Signatures: [][]byte{sig, sig, sig}})
	atts := []*qrysmpb.Attestation{att1, att2}
