...
// ProcessSyncAggregate verifies sync committee signatures signing over the previous slot block root.
...
func VerifySyncCommitteeSigs(s state.BeaconState, syncKeys []dilithium.PublicKey, syncSigs [][]byte) error {
	if len(syncKeys) != len(syncSigs) {
		return fmt.Errorf("number of pubkeys %d does not match number of signatures %d", len(syncKeys), len(syncSigs))
	}
