...
// UpdateGenesisZondData updates zond data for genesis state.
func UpdateGenesisZondData(state state.BeaconState, deposits []*zondpb.Deposit, zondData *zondpb.ZondData) (state.BeaconState, error) {
	if zondData == nil {
	...
	zondData.DepositRoot = depositRoot[:]
	err = state.SetZondData(zondData)
	if err != nil {
		return nil, err
	}
	return state, nil
