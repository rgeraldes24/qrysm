...
func TestProcessDeposits_MerkleBranchFailsVerification(t *testing.T) {
	deposit := &zondpb.Deposit{
		Data: &zondpb.Deposit_Data{
			PublicKey:             bytesutil.PadTo([]byte{1, 2, 3}, dilithiumlib.CryptoPublicKeyBytes),
...
func TestPreGenesisDeposits_SkipInvalidDeposit(t *testing.T) {
	...
	if newState.Zond1DepositIndex() != 100 {
		t.Errorf(
			"Expected Zond1DepositIndex to be increased by 99 after processing an invalid deposit, received change: %v",
			newState.Zond1DepositIndex(),
		)
	}
