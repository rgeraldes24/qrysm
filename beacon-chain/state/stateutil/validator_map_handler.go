...
type ValidatorMapHandler struct {
	valIdxMap map[[dilithium.CryptoPublicKeyBytes]byte]primitives.ValidatorIndex
	mapRef    *Reference
	*sync.RWMutex
}
...
func (v *ValidatorMapHandler) Copy() *ValidatorMapHandler {
	if v == nil || v.valIdxMap == nil {
		return &ValidatorMapHandler{valIdxMap: map[[dilithium.CryptoPublicKeyBytes]byte]primitives.ValidatorIndex{}, mapRef: new(Reference), RWMutex: new(sync.RWMutex)}
	}
	v.RLock()
	defer v.RUnlock()
	m := make(map[[dilithium.CryptoPublicKeyBytes]byte]primitives.ValidatorIndex, len(v.valIdxMap))
...
func (v *ValidatorMapHandler) Get(key [dilithium.CryptoPublicKeyBytes]byte) (primitives.ValidatorIndex, bool) {
	v.RLock()
	defer v.RUnlock()
...
func (v *ValidatorMapHandler) Set(key [dilithium.CryptoPublicKeyBytes]byte, index primitives.ValidatorIndex) {
	v.Lock()
	defer v.Unlock()
