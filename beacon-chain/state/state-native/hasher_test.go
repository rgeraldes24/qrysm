...
func TestComputeFieldRootsWithHasher_Capella(t *testing.T) {
	...
	wrappedHeader, err := blocks.WrappedExecutionPayloadHeader(executionPayloadHeaderCapella(), 0)
	require.NoError(t, err)
	...
	protoState, ok := nativeState.ToProtoUnsafe().(*zondpb.BeaconState)
	require.Equal(t, true, ok)
...
func executionPayloadHeaderCapella() *enginev1.ExecutionPayloadHeader {
	ph := bytesutil.ToBytes32([]byte("parent"))
	fr := bytesutil.PadTo([]byte("fee"), 20)
	sr := bytesutil.ToBytes32([]byte("state"))
	rr := bytesutil.ToBytes32([]byte("receipts"))
