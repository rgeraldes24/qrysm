	...
	tests := []struct {
				...
				for i := uint64(0); i < params.BeaconConfig().SyncCommitteeSubnetCount; i++ {
					...
					for _, p := range coms {
						...
						if isAggregator {
							...
							msg.Message.Contribution.Signatures = infiniteSig[:]
							msg.Message.Contribution.BlockRoot = headRoot[:]
				...
				for i := uint64(0); i < params.BeaconConfig().SyncCommitteeSubnetCount; i++ {
					...
					for _, p := range coms {
						...
						if isAggregator {
							...
							msg.Message.Contribution.Slot = slots.PrevSlot(hState.Slot())
							msg.Message.Contribution.SubcommitteeIndex = i
							msg.Message.Contribution.Signatures = badSig.Marshal()
							msg.Message.Contribution.BlockRoot = headRoot[:]
							msg.Message.Contribution.AggregationBits = bitfield.NewBitvector16()
							msg.Message.Contribution.AggregationBits.SetBitAt(1, true)
							msg.Signature = infiniteSig[:]
		...
		},
		{
			...
			setupSvc: func(s *Service, msg *zondpb.SignedContributionAndProof) (*Service, *startup.Clock) {
				...
				s.cfg.chain = &mockChain.ChainService{
					SyncCommitteeIndices:        []primitives.CommitteeIndex{primitives.CommitteeIndex(msg.Message.Contribution.SubcommitteeIndex * subCommitteeSize)},
					PublicKey:                   bytesutil.ToBytes2592(keys[msg.Message.AggregatorIndex].PublicKey().Marshal()),
					SyncSelectionProofDomain:    d,
				...
				for i := uint64(0); i < params.BeaconConfig().SyncCommitteeSubnetCount; i++ {
					...
					for _, p := range coms {
						...
						if isAggregator {
							...
							msg.Message.Contribution.Signatures = sig.Marshal()
				...
				for i := uint64(0); i < params.BeaconConfig().SyncCommitteeSubnetCount; i++ {
					...
					for _, p := range coms {
						...
						if isAggregator {
							...
							msg.Message.Contribution.Signatures = bls.AggregateSignatures(sigs).Marshal()
							sigRoot, err = signing.ComputeSigningRoot(msg.Message, cd)
							assert.NoError(t, err)
							contrSig := keys[idx].Sign(sigRoot[:])
							msg.Signature = contrSig.Marshal()
