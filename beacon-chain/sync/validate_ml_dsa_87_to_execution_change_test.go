...
func TestService_ValidateMLDSA87ToExecutionChange(t *testing.T) {
	beaconDB := testingdb.SetupDB(t)
	defaultTopic := p2p.MLDSA87ToExecutionChangeSubnetTopicFormat + "/" + encoder.ProtocolSuffixSSZSnappy
	...
	type args struct {
		pid   peer.ID
		msg   *qrysmpb.SignedMLDSA87ToExecutionChange
		topic string
	}
	tests := []struct {
		name     string
		svcopts  []Option
		setupSvc func(s *Service, msg *qrysmpb.SignedMLDSA87ToExecutionChange, topic string) (*Service, string)
		clock    *startup.Clock
		args     args
			...
			args: args{
				...
				msg: &qrysmpb.SignedMLDSA87ToExecutionChange{
					Message: &qrysmpb.MLDSA87ToExecutionChange{
						ValidatorIndex:     0,
						FromMldsa87Pubkey:  make([]byte, field_params.MLDSA87PubkeyLength),
...

		{
			...
			setupSvc: func(s *Service, msg *qrysmpb.SignedMLDSA87ToExecutionChange, topic string) (*Service, string) {
				...
				msg.Message.FromMldsa87Pubkey = keys[51].PublicKey().Marshal()
		...
		},
		{
			...
			setupSvc: func(s *Service, msg *qrysmpb.SignedMLDSA87ToExecutionChange, topic string) (*Service, string) {
				...
				msg.Message.ValidatorIndex = 50
				// Provide invalid withdrawal key for validator
				msg.Message.FromMldsa87Pubkey = keys[0].PublicKey().Marshal()
				msg.Message.ToExecutionAddress = wantedExecAddress
				return s, topic
			},
			args: args{
		...
		},
		{
			...
			setupSvc: func(s *Service, msg *qrysmpb.SignedMLDSA87ToExecutionChange, topic string) (*Service, string) {
				...
				assert.NoError(t, st.ApplyToEveryValidator(func(idx int, val *qrysmpb.Validator) (bool, *qrysmpb.Validator, error) {
					...
					newCreds[0] = params.BeaconConfig().QRLAddressWithdrawalPrefixByte
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got, err := svc.validateMLDSA87ToExecutionChange(ctx, tt.args.pid, msg); got != tt.want {
				_ = err
				t.Errorf("validateMLDSA87ToExecutionChange() = %v, want %v", got, tt.want)
			}
		})
	}
}
