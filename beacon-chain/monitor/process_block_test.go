...
func TestProcessBlock_AllEventsTrackedVals(t *testing.T) {
	...
	proposerIdx := b.Block.ProposerIndex
	trackValidatorForMonitor(svc, idx, 39999900000000)
	...
	wanted1 := fmt.Sprintf("\"Proposed beacon block was included\" BalanceChange=100000000 BlockRoot=%#x NewBalance=40000000000000 ParentRoot=%#x ProposerIndex=%d Slot=1 Version=0 prefix=monitor", bytesutil.Trunc(root[:]), bytesutil.Trunc(b.Block.ParentRoot), proposerIdx)
