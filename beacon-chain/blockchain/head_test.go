...
func TestSaveOrphanedAtts(t *testing.T) {
	...
	for _, blk := range []*qrysmpb.SignedBeaconBlockCapella{blkG, blk1, blk2, blk3, blk4} {
...
func TestSaveOrphanedAtts_CanFilter(t *testing.T) {
	...
	beaconDB := testDB.SetupDB(t)
	service := setupBeaconChain(t, beaconDB)
	service.cfg.MLDSA87ToExecPool = mldsa87toexec.NewPool()
	...
	blkConfig.NumMLDSA87Changes = 10
	...
	blkConfig.NumMLDSA87Changes = 15
	...
	for _, blk := range []*qrysmpb.SignedBeaconBlockCapella{blkG, blk1, blk2, blk4} {
		r, err := blk.Block.HashTreeRoot()
		require.NoError(t, err)
		state, blkRoot, err := prepareForkchoiceState(ctx, blk.Block.Slot, r, bytesutil.ToBytes32(blk.Block.ParentRoot), [32]byte{}, ojc, ofc)
		require.NoError(t, err)
	...
	pending, err := service.cfg.MLDSA87ToExecPool.PendingMLDSA87ToExecChanges()
	require.NoError(t, err)
