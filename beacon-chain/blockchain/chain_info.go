...
type HeadFetcher interface {
	...
	HeadZondData() *zondpb.ZondData
	HeadPublicKeyToValidatorIndex(pubKey [dilithium.CryptoPublicKeyBytes]byte) (primitives.ValidatorIndex, bool)
	HeadValidatorIndexToPublicKey(ctx context.Context, index primitives.ValidatorIndex) ([dilithium.CryptoPublicKeyBytes]byte, error)
...
// HeadZondData returns the zonddata of the current head state.
func (s *Service) HeadZondData() *zondpb.ZondData {
	s.headLock.RLock()
	defer s.headLock.RUnlock()
...
func (s *Service) HeadPublicKeyToValidatorIndex(pubKey [dilithium.CryptoPublicKeyBytes]byte) (primitives.ValidatorIndex, bool) {
	s.headLock.RLock()
	defer s.headLock.RUnlock()
	if !s.hasHeadState() {
		return 0, false
...
func (s *Service) HeadValidatorIndexToPublicKey(_ context.Context, index primitives.ValidatorIndex) ([dilithium.CryptoPublicKeyBytes]byte, error) {
	s.headLock.RLock()
	defer s.headLock.RUnlock()
	if !s.hasHeadState() {
		return [dilithium.CryptoPublicKeyBytes]byte{}, nil
	}
