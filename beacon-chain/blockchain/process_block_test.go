...
func TestInsertFinalizedDeposits(t *testing.T) {
	...
	require.NoError(t, service.saveGenesisData(ctx, gs))
	gs = gs.Copy()
	assert.NoError(t, gs.SetZondData(&zondpb.ZondData{DepositCount: 10, BlockHash: make([]byte, 32)}))
	assert.NoError(t, gs.SetZondDepositIndex(8))
...
func TestInsertFinalizedDeposits_MultipleFinalizedRoutines(t *testing.T) {
	...
	require.NoError(t, service.saveGenesisData(ctx, gs))
	gs = gs.Copy()
	assert.NoError(t, gs.SetZondData(&zondpb.ZondData{DepositCount: 7, BlockHash: make([]byte, 32)}))
	assert.NoError(t, gs.SetZondDepositIndex(6))
	assert.NoError(t, service.cfg.StateGen.SaveState(ctx, [32]byte{'m', 'o', 'c', 'k'}, gs))
	gs2 := gs.Copy()
	assert.NoError(t, gs2.SetZondData(&zondpb.ZondData{DepositCount: 15, BlockHash: make([]byte, 32)}))
	assert.NoError(t, gs2.SetZondDepositIndex(13))
