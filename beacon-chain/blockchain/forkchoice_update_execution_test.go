...
func TestService_forkchoiceUpdateWithExecution_SameHeadRootNewProposer(t *testing.T) {
	...
	require.NoError(t, err)
	bellatrixBlk := util.SaveBlock(t, ctx, beaconDB, util.NewBeaconBlock())
	...
	sb, err := blocks.NewSignedBeaconBlock(util.HydrateSignedBeaconBlock(&zondpb.SignedBeaconBlock{}))
	require.NoError(t, err)
