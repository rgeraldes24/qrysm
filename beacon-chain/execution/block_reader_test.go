...
func TestLatestMainchainInfo_OK(t *testing.T) {
	...
	web3Service.zondHeadTicker = &time.Ticker{C: tickerChan}
	tickerChan <- time.Now()
	web3Service.cancel()
	exitRoutine <- true

	assert.Equal(t, web3Service.latestZondData.BlockHeight, header.Number.Uint64())
	assert.Equal(t, hexutil.Encode(web3Service.latestZondData.BlockHash), header.Hash.Hex())
	assert.Equal(t, web3Service.latestZondData.BlockTime, header.Time)
}
...
func TestBlockHashByHeight_ReturnsError_WhenNoZondClient(t *testing.T) {
	beaconDB := dbutil.SetupDB(t)
	server, endpoint, err := mockExecution.SetupRPCServer()
	require.NoError(t, err)
	t.Cleanup(func() {
...
func TestService_BlockNumberByTimestamp(t *testing.T) {
	...
	require.NoError(t, err)
	web3Service.latestZondData.BlockTime = hd.Time
	web3Service.latestZondData.BlockHeight = hd.Number.Uint64()
...
func TestService_BlockNumberByTimestampLessTargetTime(t *testing.T) {
	...
	_, err = web3Service.findMaxTargetZondBlock(ctx, hd.Number, hd.Time/2)
	require.ErrorContains(t, context.DeadlineExceeded.Error(), err)

	// Provide an attainable target time
	blk, err := web3Service.findMaxTargetZondBlock(context.Background(), hd.Number, hd.Time-5)
	require.NoError(t, err)
...
func TestService_BlockNumberByTimestampMoreTargetTime(t *testing.T) {
	...
	_, err = web3Service.findMinTargetZondBlock(ctx, big.NewInt(0).Div(hd.Number, big.NewInt(2)), hd.Time)
	require.ErrorContains(t, context.DeadlineExceeded.Error(), err)

	// Provide an attainable target time with respect to head
	blk, err := web3Service.findMinTargetZondBlock(context.Background(), big.NewInt(0).Sub(hd.Number, big.NewInt(5)), hd.Time)
	require.NoError(t, err)
	require.Equal(t, hd.Number.Uint64(), blk.Number.Uint64(), "retrieved block is not equal to the head")
}

func TestService_BlockTimeByHeight_ReturnsError_WhenNoZondClient(t *testing.T) {
	beaconDB := dbutil.SetupDB(t)
	server, endpoint, err := mockExecution.SetupRPCServer()
	require.NoError(t, err)
	t.Cleanup(func() {
