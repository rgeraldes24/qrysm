...
func TestServer_FeeRecipientByPubkey(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err = s.SetFeeRecipientByPubkey(ctx, &zondpbservice.SetFeeRecipientByPubkeyRequest{Pubkey: byteval, Zondaddress: common.HexToAddress(tt.args).Bytes()})
			require.NoError(t, err)
...
func TestServer_DeleteFeeRecipientByPubkey(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want: &want{
				ZondAddress: common.HexToAddress("Z046Fb65722E7b2455012BFEBf6177F1D2e9738D9").Hex(),
			},
			wantErr: false,
		},
	}
