...
func TestServer_ListFeeRecipientByPubkey(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			wantAddr, err := common.NewAddressFromString(tt.want.QRLAddress)
			require.NoError(t, err)
			assert.Equal(t, wantAddr.Hex(), common.BytesToAddress(got.Data.Qrladdress).Hex())
		})
	}
}
...
func TestServer_FeeRecipientByPubkey(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			expectedAddr, err := common.NewAddressFromString(tt.want.valQRLAddress)
			require.NoError(t, err)
			assert.Equal(t, expectedAddr.Hex(), s.validatorService.ProposerSettings().ProposeConfig[bytesutil.ToBytes2592(byteval)].FeeRecipientConfig.FeeRecipient.Hex())
		})
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			exit := &qrysmpb.VoluntaryExit{
				Epoch:          primitives.Epoch(resp.Data.Message.Epoch),
				ValidatorIndex: primitives.ValidatorIndex(resp.Data.Message.ValidatorIndex),
			}
			require.NoError(t, signing.VerifySigningRoot(exit, pubKeys[0][:], resp.Data.Signature, make([]byte, 32)))
		})
	}
}
