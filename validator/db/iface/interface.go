...
type ValidatorDB interface {
	...
	UpdatePublicKeysBuckets(publicKeys [][field_params.DilithiumPubkeyLength]byte) error
	...
	HighestSignedProposal(ctx context.Context, publicKey [field_params.DilithiumPubkeyLength]byte) (primitives.Slot, bool, error)
	LowestSignedProposal(ctx context.Context, publicKey [field_params.DilithiumPubkeyLength]byte) (primitives.Slot, bool, error)
	ProposalHistoryForPubKey(ctx context.Context, publicKey [field_params.DilithiumPubkeyLength]byte) ([]*kv.Proposal, error)
	ProposalHistoryForSlot(ctx context.Context, publicKey [field_params.DilithiumPubkeyLength]byte, slot primitives.Slot) ([32]byte, bool, error)
	SaveProposalHistoryForSlot(ctx context.Context, pubKey [field_params.DilithiumPubkeyLength]byte, slot primitives.Slot, signingRoot []byte) error
	ProposedPublicKeys(ctx context.Context) ([][field_params.DilithiumPubkeyLength]byte, error)

	// Attester protection related methods.
	// Methods to store and read blacklisted public keys from EIP-3076
	// slashing protection imports.
	EIPImportBlacklistedPublicKeys(ctx context.Context) ([][field_params.DilithiumPubkeyLength]byte, error)
	SaveEIPImportBlacklistedPublicKeys(ctx context.Context, publicKeys [][field_params.DilithiumPubkeyLength]byte) error
	SigningRootAtTargetEpoch(ctx context.Context, publicKey [field_params.DilithiumPubkeyLength]byte, target primitives.Epoch) ([32]byte, error)
	LowestSignedTargetEpoch(ctx context.Context, publicKey [field_params.DilithiumPubkeyLength]byte) (primitives.Epoch, bool, error)
	...
	UpdateProposerSettingsForPubkey(context.Context, [field_params.DilithiumPubkeyLength]byte, *validatorServiceConfig.ProposerOption) error
