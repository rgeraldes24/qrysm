...
func Test_pubKeyFromHex(t *testing.T) {
	tests := []struct {
		name    string
		str     string
		want    [dilithium.CryptoPublicKeyBytes]byte
		wantErr bool
	}{
		{
		...
		},
		{
			...
			want: [dilithium.CryptoPublicKeyBytes]byte{184, 69, 8, 154, 20, 87, 248, 17, 191, 192, 0, 88, 143, 187, 78, 113, 54, 105, 190, 140, 224, 96, 234, 107, 227, 198, 236, 224, 154, 252, 55, 148, 16, 108, 145, 202, 115, 172, 218, 94, 84, 87, 18, 45, 88, 114, 59, 237},
		},
		{
...
func Test_pubKeyToHexString(t *testing.T) {
	mockPubKey := [dilithium.CryptoPublicKeyBytes]byte{1}
	tests := []struct {
		name    string
		pubKey  []byte
		want    string
