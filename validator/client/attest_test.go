...
func TestAttestToBlockHead_AttestsCorrectly(t *testing.T) {
	...
	require.NoError(t, signing.VerifySigningRoot(
		expectedAttestation.Data,
		validatorKey.PublicKey().Marshal(),
		generatedAttestation.Signatures[0],
		make([]byte, 32),
	))
...
func TestSignAttestation(t *testing.T) {
	...
	require.Equal(t, field_params.MLDSA87SignatureLength, len(sig))
	require.NoError(t, signing.VerifySigningRoot(att.Data, pk.pub[:], sig, attesterDomain))
	// proposer domain
