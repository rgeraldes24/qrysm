...
def process_epoch(state: BeaconState) -> None:
    ...
    process_execution_data_reset(state)
...
def get_base_reward(state: BeaconState, index: ValidatorIndex) -> Shor:
    ...
    return Shor(effective_balance * BASE_REWARD_FACTOR // integer_squareroot(total_balance) // BASE_REWARDS_PER_EPOCH)
```
```python
def get_proposer_reward(state: BeaconState, attesting_index: ValidatorIndex) -> Shor:
    return Shor(get_base_reward(state, attesting_index) // PROPOSER_REWARD_QUOTIENT)
```
```python
...
def get_attestation_component_deltas(state: BeaconState,
                                     ...
                                     ) -> Tuple[Sequence[Shor], Sequence[Shor]]:
    """
    Helper with shared logic for use by get source, target, and head deltas functions
    """
    rewards = [Shor(0)] * len(state.validators)
    penalties = [Shor(0)] * len(state.validators)
    total_balance = get_total_active_balance(state)
...
```
```python
def get_source_deltas(state: BeaconState) -> Tuple[Sequence[Shor], Sequence[Shor]]:
    """
...
```
```python
def get_target_deltas(state: BeaconState) -> Tuple[Sequence[Shor], Sequence[Shor]]:
    """
...
```
```python
def get_head_deltas(state: BeaconState) -> Tuple[Sequence[Shor], Sequence[Shor]]:
    """
...
```
```python
def get_inclusion_delay_deltas(state: BeaconState) -> Tuple[Sequence[Shor], Sequence[Shor]]:
    """
    Return proposer and inclusion delay micro-rewards/penalties for each validator.
    """
    rewards = [Shor(0) for _ in range(len(state.validators))]
    matching_source_attestations = get_matching_source_attestations(state, get_previous_epoch(state))
    for index in get_unslashed_attesting_indices(state, matching_source_attestations):
        ...
        max_attester_reward = Shor(get_base_reward(state, index) - get_proposer_reward(state, index))
        rewards[index] += Shor(max_attester_reward // attestation.inclusion_delay)

    # No penalties associated with inclusion delay
    penalties = [Shor(0) for _ in range(len(state.validators))]
    return rewards, penalties
```
```python
def get_inactivity_penalty_deltas(state: BeaconState) -> Tuple[Sequence[Shor], Sequence[Shor]]:
    """
    ...
    if is_in_inactivity_leak(state):
        ...
        for index in get_eligible_validator_indices(state):
            ...
            penalties[index] += Shor(BASE_REWARDS_PER_EPOCH * base_reward - get_proposer_reward(state, index))
            if index not in matching_target_attesting_indices:
                effective_balance = state.validators[index].effective_balance
                penalties[index] += Shor(effective_balance * get_finality_delay(state) // INACTIVITY_PENALTY_QUOTIENT)
...
```
```python
def get_attestation_deltas(state: BeaconState) -> Tuple[Sequence[Shor], Sequence[Shor]]:
    """
