...
func DefaultDataDir() string {
	...
	if home != "" {
		if runtime.GOOS == "darwin" {
			return filepath.Join(home, "Library", "Qrysm")
		} else if runtime.GOOS == "windows" {
			return filepath.Join(home, "AppData", "Local", "Qrysm")
		} else {
			return filepath.Join(home, ".qrysm")
		}
	}
...
func FixDefaultDataDir(prevDataDir, curDataDir string) error {
	...
	if !prevDataDirExists {
		// If no previous "%APPDATA%\Qrysm" found, nothing to patch and move to new default location.
		return nil
	}
