...
func TestDepositInput_GeneratesPb(t *testing.T) {
	...
	k1, err := cryptodilithium.SecretKeyFromBytes(seed[:])
	require.NoError(t, err)

	_, err = rand.Read(seed[:])
	require.NoError(t, err)
	k2, err := cryptodilithium.SecretKeyFromBytes(seed[:])
	require.NoError(t, err)
	...
	sig, err := cryptodilithium.SignatureFromBytes(result.Signature)
	require.NoError(t, err)
