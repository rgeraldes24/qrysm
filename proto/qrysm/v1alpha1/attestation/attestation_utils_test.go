...
func TestVerifyIndexedAttestationSigs(t *testing.T) {
	...
	_ = pk0
	_ = pk1
	sk0, err := ml_dsa_87.RandKey()
	require.NoError(t, err)
	sk1, err := ml_dsa_87.RandKey()
	require.NoError(t, err)
	pk0 = sk0.PublicKey()
	pk1 = sk1.PublicKey()
	testAttData := &qrysmpb.AttestationData{
		Slot:            5,
		CommitteeIndex:  2,
		BeaconBlockRoot: []byte("11111111111111111111111111111111"),
		Source: &qrysmpb.Checkpoint{
			Epoch: 4,
			Root:  []byte("11111111111111111111111111111111"),
		},
		Target: &qrysmpb.Checkpoint{
			Epoch: 10,
			Root:  []byte("11111111111111111111111111111111"),
	...
	signingRoot, err := signing.ComputeSigningRoot(testAttData, []byte("11111111111111111111111111111111"))
	require.NoError(t, err)
