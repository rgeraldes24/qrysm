...
// Hash: 56ab534c9766f3c1f939637447e58650f08643232fc2fd1929a01a62446503b8
package zond
...
func (a *Attestation) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Offset (3) 'SignaturesIdxToValidatorIdx'
	dst = ssz.WriteOffset(dst, offset)
	offset += len(a.SignaturesIdxToValidatorIdx) * 8
	...
	// Field (3) 'SignaturesIdxToValidatorIdx'
	if size := len(a.SignaturesIdxToValidatorIdx); size > 2048 {
		err = ssz.ErrListTooBigFn("--.SignaturesIdxToValidatorIdx", size, 2048)
		return
	}
	for ii := 0; ii < len(a.SignaturesIdxToValidatorIdx); ii++ {
		dst = ssz.MarshalUint64(dst, a.SignaturesIdxToValidatorIdx[ii])
	}
...
func (a *Attestation) UnmarshalSSZ(buf []byte) error {
	...
	// Field (3) 'SignaturesIdxToValidatorIdx'
	{
		...
		a.SignaturesIdxToValidatorIdx = ssz.ExtendUint64(a.SignaturesIdxToValidatorIdx, num)
		for ii := 0; ii < num; ii++ {
			a.SignaturesIdxToValidatorIdx[ii] = ssz.UnmarshallUint64(buf[ii*8 : (ii+1)*8])
		}
	}
	return err
}
...
func (a *Attestation) SizeSSZ() (size int) {
	...
	// Field (3) 'SignaturesIdxToValidatorIdx'
	size += len(a.SignaturesIdxToValidatorIdx) * 8
...
func (a *Attestation) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	...
	// Field (3) 'SignaturesIdxToValidatorIdx'
	{
		...
		subIndx := hh.Index()
		for _, i := range a.SignaturesIdxToValidatorIdx {
			hh.AppendUint64(i)
		}
		hh.FillUpTo32()

		numItems := uint64(len(a.SignaturesIdxToValidatorIdx))
		if ssz.EnableVectorizedHTR {
